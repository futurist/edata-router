...
(function (global, factory) {
    ...
    function () {
      ...
      _createClass(mitt, [{
        ...
        value: function emit(type, evt) {
          ...
          (this._events['*'] || []).slice().map(function (handler) {
            handler(type, evt);
          });
        }
      }]);
  ...
  var EventEmitter = unwrapExports(src);
  ...
  function (_EventEmitter) {
    _inherits(EdataBaseClass, _EventEmitter);

    function EdataBaseClass() {
      var _this;

      _classCallCheck(this, EdataBaseClass);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(EdataBaseClass).call(this));
      if (arguments.length > 0) _this._value = arguments[0];
      return _this;
    }

    _createClass(EdataBaseClass, [{
      key: "map",
  ...
  function isPOJO(x) {
    ...
    return toString.call(x) === '[object Object]' && (prototype = getPrototypeOf$$1(x), prototype === null || prototype === getPrototypeOf$$1({}));
  }
  ...
  function isPrimitive(val) {
    return val == null || typeof val !== 'function' && _typeof(val) !== 'object';
  }

  function getPath(path) {
    return isArray(path) ? path.map(function (p) {
      return isArray(p) ? p : typeof p === 'number' ? [true, p] : [false, p];
    }) : typeof path === 'number' ? [[true, path]] : stringToPath(String(path));
  }

  function edata(initData) {
    var config = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    var BaseClass = config.baseClass,
        _config$unwrapConfig = config.unwrapConfig,
        unwrapConfig = _config$unwrapConfig === void 0 ? null : _config$unwrapConfig,
        _config$plugins = config.plugins,
        plugins = _config$plugins === void 0 ? [] : _config$plugins;
    BaseClass = BaseClass || EdataBaseClass;
    plugins.unshift(combine);

    var ObserverClass =
    /*#__PURE__*/
    function (_BaseClass) {
      _inherits(ObserverClass, _BaseClass);

      function ObserverClass(packed) {
        var _this3;

        _classCallCheck(this, ObserverClass);

        _this3 = _possibleConstructorReturn(this, _getPrototypeOf(ObserverClass).call(this));
        _this3.changeStack = [];
        _this3._packed = packed;
        _this3._skip = false;
        _this3._hold = false;
        _this3.meta = {};
        _this3.count = 0;
      ...
      _createClass(ObserverClass, [{
        key: "skip",
        get: function get() {
          return this._skip;
        },
        set: function set(_skip) {
          this._skip = _skip;
        ...
        set: function set(_hold) {
          var _this4 = this;

          this._hold = _hold;

          if (!_hold) {
            this.changeStack.forEach(function (v) {
              _this4.value = v;
            });
            this.changeStack = [];
          }
        ...
        set: function set(_ref) {
          var data = _ref.data,
              type = _ref.type,
              meta = _ref.meta,
              changeArgs = _ref.changeArgs;
          var root = this._packed.root;
          ...
          var baseVal = {
            data: data,
            type: type,
            meta: _objectSpread2({}, this.meta, {}, meta)
          };
    ...
    var wrapper = function wrapper(init) {
      return new BaseClass(init);
    };

    var isWrapper = function isWrapper(obj) {
    ...
    function _checkCacheAndUnwrap(config, _cache, val, result, key) {
      var prev = _cache.get(val);

      if (prev != null) {
        !config.json && prev.push(function () {
    ...
    function _unwrap(obj, config, _cache) {
      ...
      if (isWrappedData(source)) {
        ...
        result = [];
        source.forEach(function (val, key) {
    ...
    function wrapSource(source) {
      ...
      function deepIt(a, b, callback, path) {
        ...
        for (var key in b) {
          if (!hasOwnProperty.call(b, key)) continue; // return false stop the iteration

          var ret = callback(a, b, key, path, _cache);
          if (ret === false) break;else if (ret === 0) continue;
          var aval = a[key];
          var bval = b[key];

          if (!isPrimitive2(bval) && isWrapper(aval) && !isPrimitive(aval.value)) {
            var prev = _cache.get(bval);
      ...
      function watch(path, fn) {
        ...
        var _this$cut = this.cut(),
            observer = _this$cut.observer;

        return path == null ? observer.map(fn) : observer.map(function (e) {
          var p = e.path.join('.');
      ...
      function cut(path, from, filter) {
        ...
        var part = makeChange(targetObj);
        if (!isWrapper(part)) return part;
        var observer = part.observer;
        var targetRoot = from || root;
        var subPath = targetObj.path; // const subPath = getPath(path).map(v => v[1])

        if (!isFunction(filter)) {
          filter = function filter(arg) {
            return arg.path.join().indexOf(arg.subPath.join()) === 0;
          };
        }

        var watchFn = function watchFn(_ref2) {
          var data = _ref2.data,
              type = _ref2.type,
              meta = _ref2.meta,
              path = _ref2.path;
      ...
      function makeChange(packed) {
        ...
        var _change = new ObserverClass(packed); // const oldMap = _change.map
      ...
      function bindMethods(packed, path) {
        var type = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : MUTATION_TYPE.UPDATE;
        if ('path' in packed && 'root' in packed) return packed; // type: 0->CHANGE, 1->ADD, 2->DELETE
        ...
        packed.wrap = function (val) {
          return createWrap(val, path);
        };
        ...
        packed.toJSON = function () {
          return packed.unwrap({
            json: true
          });
        };
      ...
      function createWrap(source) {
        ...
        var target = isArray(source) ? [] : isPOJO(source) ? {} : source;
        packed.value = deepIt(target, source, function (a, b, key, path) {
          var _path = path.concat({
            key: key
          });

          var bval = b[key];
          if (bval === undefined) a[key] = wrapper();else if (shouldNotDig(bval)) a[key] = wrapper(bval);else {
            var prev = _cache.get(bval);
        ...
        var ret = bindMethods(packed, prevPath);
      ...
      function closest(path) {
        ...
        var lastPos = -1;
        var obj = this;
        var reg = new RegExp(path instanceof RegExp ? path : '\\b' + path + '\\b', 'g');
        var pathString = obj.path.slice(0, -1).join('.');
        ...
        if (lastPos > -1) {
          var pos = pathString.indexOf('.', lastPos);
          return root.get(pos > -1 ? pathString.slice(0, pos) : pathString);
        }
      }
      ...
      function getset(path, func) {
        ...
        if (!path.length) {
          ...
          for (i = 0, len = path.length - 1; i < len; i++) {
            ...
            var _path2 = _slicedToArray(path[i + 1], 1);

            nextIsArray = _path2[0];

            if (!isWrapper(n[p])) {
              n[p] = bindMethods(wrapper(nextIsArray ? [] : {}), _path.slice(0, i + 1), MUTATION_TYPE.CREATE);
            }
          ...
          if (isWrapper(n[p])) {
            ...
            if (isPrimitive(descriptor)) {
              ...
              Object.defineProperty(n, p, assign({
                value: value
              }, descriptor));
            }
      ...
      function unset(path) {
        ...
        if (path == null) {
          var _p = getPath(obj.path);

          path = [_p.pop()];
          obj = root.get(_p); // console.log(333, obj);
        } else {
          path = getPath(path);
        }

        var len = path.length;
        var val = obj.get(path.slice(0, -1));
        if (val == null) return;
        var parent = val.value;
        /* eslint-disable-next-line no-unused-vars */

        var _path3 = _slicedToArray(path[len - 1], 2),
            t = _path3[0],
            p = _path3[1];

        if (!(p in parent)) return;
        var deleteVal = parent[p];
        delete parent[p];
        root.observer.value = {
          data: deleteVal,
      ...
      function proxy(path, config) {
        ...
        var obj = path != null ? this.get(path) : this;
        return obj == null ? obj : observe(obj, _objectSpread2({}, config));
      }
      ...
      function observe(edata, config) {
        function buildProxy(o, thisObject) {
          ...
          var oIsEdata = isWrapper(o);
          var _target = o;

          while (isWrapper(_target)) {
            _target = _target.value;
          }

          var isObject = _target instanceof Object;
          ...
          return new Proxy(_target, {
            ...
            get: function get(target, property) {
              ...
              if (typeof out === 'function') {
                var isEdataMethod = oIsEdata && typeof o[property] === 'function' && property !== 'map';
                return function () {
                  for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
                    args[_key] = arguments[_key];
                  }

                  var ret = isEdataMethod ? o[property].apply(o, args) : out.apply(thisObject || (Array.isArray(target) ? target.map(function (v) {
                    return buildProxy(v);
                  }) : buildProxy(target)), args);
      ...
      function copyWithin(target, start, end) {
        ...
        var copyArr = value.slice(start, end);
        var fromIndex = target;
        var toIndex = target + copyArr.length;
        value.slice(fromIndex, toIndex).forEach(function (item, i) {
          _this5.set(i, copyArr[i].value);
        });
        return this;
      }

      function fill(value, start, end) {
        var copyArr = this.value.slice(start, end);
      ...
      function reverse() {
        var value = this.value;
        value.reverse().forEach(function (item, i) {
      ...
      function sort(compareFunction) {
        var value = this.value;
        value.sort(compareFunction).forEach(function (item, i) {
          item._path[item._path.length - 1].key = i;
          item.set(item.value);
        });
        return this;
      ...
      function splice(start, deleteCount) {
        ...
        value.splice.apply(value, [start, deleteCount].concat(args));
        args.forEach(function (val, i) {
          _this6.set(start + i, val);
        });
        var delta = args.length - deleteCount;

        if (delta) {
          for (var _i = start + args.length; _i < value.length; _i++) {
            var item = value[_i];
            var path = item._path[item._path.length - 1];
      ...
      function unshift() {
        ...
        var value = this.value;
        var length = value.unshift.apply(value, arguments);
      ...
      function unwrap(path) {
        ...
        var obj = path != null ? this.get(path) : this;
        return _unwrap(obj, _objectSpread2({}, isFunction(unwrapConfig) && unwrapConfig(obj), {}, config));
      }
  ...
  function edataProxy(obj) {
    var config = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    return edata(obj, _objectSpread2({}, config, {
      isProxy: true
    })).proxy();
  }
  edata.version = '0.9.5';

  exports.EdataBaseClass = EdataBaseClass;
  exports.default = edata;
  exports.edataProxy = edataProxy;
...
var baseSet = __webpack_require__(67);
...
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(9), __webpack_require__(108)(module)))
...
var MediaType = __webpack_require__(20)
...
var memoizeCapped = __webpack_require__(79);
...
// EXTERNAL MODULE: ./node_modules/edata/dist/umd.js
...
var is_plain_obj = __webpack_require__(35);
...
var url_polyfill = __webpack_require__(105);

// EXTERNAL MODULE: ./node_modules/abortcontroller-polyfill/dist/abortcontroller-polyfill-only.js
var abortcontroller_polyfill_only = __webpack_require__(106);
...
function fetch_Response(bodyInit, options) {
  if (!options) {
    options = {}
  }
...
Body.call(fetch_Response.prototype)

fetch_Response.prototype.clone = function() {
  return new fetch_Response(this._bodyInit, {
    status: this.status,
...
fetch_Response.error = function() {
  var response = new fetch_Response(null, {status: 0, statusText: ''})
  response.type = 'error'
  return response
}
...
fetch_Response.redirect = function(url, status) {
  if (redirectStatuses.indexOf(status) === -1) {
    throw new RangeError('Invalid status code')
  }

  return new fetch_Response(null, {status: status, headers: {location: url}})
}
...
function fetch(input, init) {
  return new Promise(function(resolve, reject) {
    ...
    xhr.onload = function() {
      ...
      resolve(new fetch_Response(body, options))
    }
...
if (!self.fetch) {
  ...
  self.Response = fetch_Response
}

// EXTERNAL MODULE: ./node_modules/fetch-parse/index.js
var fetch_parse = __webpack_require__(36);

// EXTERNAL MODULE: ./node_modules/medium-type/index.js
var medium_type = __webpack_require__(20);
var medium_type_default = /*#__PURE__*/__webpack_require__.n(medium_type);
...
var WILDCARD_PARSER = [[new medium_type_default.a("*/*"), null]];
function noop() {}
...
function replaceParams(url, params, options) {
  return path_to_regexp_default.a.compile(url)(params || {}, options);
}

function defaultGetResponse(response) {
  return Object(fetch_parse["parse"])(WILDCARD_PARSER, response);
}
...
function unwrapEData(edata) {
  while (edata instanceof umd["EdataBaseClass"]) {
    edata = edata.value;
  }
...
function makeAPI(model, res) {
  var namespace = model.name || model.displayName;

  if (!namespace) {
    throw "model should have .name or .displayName: ".concat(JSON.stringify(model));
  }
  ...
  return function (data) {
    ...
    data._api = data._api || {};
    data._store[namespace] = new umd["EdataBaseClass"](_objectSpread({}, model.store, {}, unwrapEData(data._store[namespace])));
    data._actions[namespace] = new umd["EdataBaseClass"](_objectSpread({}, model.actions, {}, unwrapEData(data._actions[namespace])));
    forEach_default()(res, function (value, key) {
      set_default()(data._api, [namespace, key], value);
    });
    var apis = data._api[namespace] || {};
    forEach_default()(model.actions, function (value, key) {
      if (!(key in apis)) {
        set_default()(data._api, [namespace, key], {});
      }
    });
    return data;
  };
}
function initModel(config, unwrapOptions) {
  return function (data) {
    return umd_default()(data, _objectSpread({
          ...
          return function (query) {
            ...
            return Promise.resolve(isFunction(apiConfig) ? apiConfig() : apiConfig).then(function (apiConfig) {
              ...
              if (!hasBody && !isEmpty(query)) {
                url = url + '?' + lib_default.a.stringify(query);
              }
              ...
              var timeoutPromise = new Promise(function (resolve, reject) {
                if (timeout > 0) {
                  timeoutId = setTimeout(function () {
                    ...
                    if (mock) {
                      var abortError = new Error('Aborted due to timeout');
              ...
              return Promise.resolve(startPromise).then(function (startStore) {
                if (startStore != null) {
...
// CONCATENATED MODULE: ./src/match-path.js
function _toArray(arr) { return match_path_arrayWithHoles(arr) || match_path_iterableToArray(arr) || match_path_nonIterableRest(); }

function match_path_nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance"); }

function match_path_iterableToArray(iter) { if (Symbol.iterator in Object(iter) || Object.prototype.toString.call(iter) === "[object Arguments]") return Array.from(iter); }

function match_path_arrayWithHoles(arr) { if (Array.isArray(arr)) return arr; }
...
/* harmony default export */ var match_path = (matchPath);
// CONCATENATED MODULE: ./node_modules/react-redux/es/components/Context.js

var ReactReduxContext = external_react_default.a.createContext(null);
...
var useSelector_useIsomorphicLayoutEffect = typeof window !== 'undefined' ? external_react_["useLayoutEffect"] : external_react_["useEffect"];
...
var src_createBrowserHistory = /* Cannot get final name for export "createHistory" in "./node_modules/history/esm/history.js" (known exports: createBrowserHistory createHashHistory createMemoryHistory createLocation locationsAreEqual parsePath createPath, known reexports: ) */ undefined || createBrowserHistory;
var src_createHashHistory = createHashHistory;

var src_EdataRouterClass =
/*#__PURE__*/
function () {
  function EdataRouterClass() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        ...
        routeMode = _ref$routeMode === void 0 ? 'hash' : _ref$routeMode,
  ...
  _createClass(EdataRouterClass, [{
    key: "model",
    value: function model(modelActions, modelObject) {
      if (typeof modelActions === 'function') {
        modelActions(this.data);
      } else {
        // makeAPI({name: '_global', ...modelActions}, modelObject)(this.data)
        makeAPI(src_objectSpread({}, modelActions), modelObject)(this.data);
      }
    }
  }, {
    key: "route",
    ...
    value: function run() {
      ...
      var model = this.model = window.model = this.makeModel(data);
      var isHashMode = routeMode === 'hash'; // const Router = isHashMode ? HashRouter : BrowserRouter

      var history = this.history = (isHashMode ? src_createHashHistory : src_createBrowserHistory)({// getUserConfirmation: (message, callback) => callback(window.confirm(message))
      });
      var curLocation = history.location || window.location;
      ...
      function expandAPINameItem(val) {
        ...
        if (val instanceof RegExp) {
          names = allAPI.filter(function (v) {
            return val.test(v);
          });
        }
      ...
      function getAPIFromRoute(_ref2) {
        var _ref2$api = _ref2.api,
            api = _ref2$api === void 0 ? [] : _ref2$api;
        var props = {}; // const apiObj = model.unwrap(['_api', '_global']) || {}
        // Object.keys(apiObj).forEach((key) => {
        //   props[key] = model.unwrap(['_api', '_global', key])
        // })
        // props.store = model.unwrap(['_store', '_global']) || {}

        api.forEach(function (val) {
          var names = expandAPINameItem(val);
      ...
      function getPathForComponent(route) {
        var path = route.path,
            ...
            _route$api = route.api,
            api = _route$api === void 0 ? [] : _route$api;
        ...
        if (path in componentMap) {
          ...
          var mapDispatchToProps = function mapDispatchToProps(dispatch, ownProps) {
            var props = getAPIFromRoute(route);
            Object.keys(props).map(function (name) {
              var service = props[name];

              var _loop = function _loop(_name) {
                var f = service[_name];

                if (typeof f === 'function') {
                  service[_name] = function () {
                    for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
                      args[_key] = arguments[_key];
                    }
          ...
          return componentMap[path] = connect_connect(mapStateToProps, mapDispatchToProps, null, {
            pure: false
          })(component);
        }
      }
      ...
      function RouteWithSubRoutes(route) {
        ...
        return external_react_default.a.createElement(external_react_router_["Route"], {
          path: route.path,
          exact: route.exact,
          strict: route.strict,
          render: function render(props) {
            // console.log(props)
            var childRoutes = route.routes || route.childRoutes || route.children;
            props.location.query = lib_default.a.parse(props.location.search.slice(1));
            var RouteComponent = getPathForComponent(route);
            return (// pass the sub-routes down to keep nesting
              external_react_default.a.createElement(RouteComponent, src_extends({}, props, {
                routeParams: props.match.params || {},
                model: subModule
              }), external_react_default.a.createElement(external_react_router_["Switch"], null, childRoutes && childRoutes.map(function (childRoute, i) {
                var path = joinPath(route.path, childRoute.path);
                return external_react_default.a.createElement(RouteWithSubRoutes, src_extends({
                  key: i
                }, childRoute, {
                  path: path,
                  model: subModule
      ...
      function computeLocationHooks(location) {
        var branch = matchRoutes(routes, location.pathname);

        var getState = function getState(v, location, routes) {
          if (!location.query) {
            location.query = lib_default.a.parse(location.search.slice(1));
          }

          return {
            location: src_objectSpread({}, location),
            params: (v.match || {}).params || {},
            routes: routes.map(function (v) {
              return v.route;
            })
          };
        };

        var exitList = routeDifference(curHooksBranch, branch);
        exitList.reverse().forEach(function (v) {
          // console.log('----onLeave')
          v.route.onLeave && v.route.onLeave.call(v.route, getState(v, curLocation, curHooksBranch));
        });
        branch.forEach(function (v) {
          var route = v.route;
          var prevState = getState(v, curLocation, curHooksBranch);
          var nextState = getState(v, location, branch);
          route.onChange && route.onChange.call(route, prevState, nextState, history.replace);
          var found = curHooksBranch.find(function (x) {
      ...
      function (_React$Component) {
        ...
        function App() {
          ...
          src_defineProperty(_assertThisInitialized(_this), "getChildContext", function () {
            return {
              router: src_objectSpread({}, history, {
                isActive: function isActive(pathname) {
                  pathname = pathname.trim().replace(/\/$/, '');
                  return curHooksBranch.some(function (e) {
                    return ((e.match || {}).path || '').indexOf(pathname) === 0;
                  });
                }
              })
            };
        ...
        _createClass(App, [{
          ...
          value: function render() {
            return external_react_default.a.createElement(components_Provider, {
              store: store
            }, external_react_default.a.createElement(external_react_router_["Router"], {
              history: history
            }, external_react_default.a.createElement(external_react_router_["Switch"], null, routes.map(function (route, i) {
              return external_react_default.a.createElement(RouteWithSubRoutes, src_extends({
                key: i
              }, route, {
                model: model
...
function matchRoutes(routes, pathname) {
  ...
  var _options$branch = options.branch,
      branch = _options$branch === void 0 ? [] : _options$branch;
  routes.some(function (route) {
    var fullPath = joinPath(options.path, route.path);
    var match = route.path ? match_path(pathname, src_objectSpread({}, route, {
      path: fullPath
    ...
    if (match) {
      ...
      if (childRoutes) {
        matchRoutes(childRoutes, pathname, {
          branch: branch,
          path: fullPath
        });
      }
